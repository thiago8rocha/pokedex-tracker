...
class PokopiaHabitatDetailScreen extends StatefulWidget {
  ...
  const PokopiaHabitatDetailScreen({
    super.key,
  ...
  @override
  State<PokopiaHabitatDetailScreen> createState() =>
      _PokopiaHabitatDetailScreenState();
}

class _PokopiaHabitatDetailScreenState
    extends State<PokopiaHabitatDetailScreen> {
  final PokeApiService _api = PokeApiService();
  final StorageService _storage = StorageService();

  // Dados carregados dos Pokémon do habitat
  ...
  Future<void> _loadPokemon() async {
    final ids =
        widget.habitat.pokemon.map((e) => e.speciesId).toList();
    if (ids.isEmpty) {
    ...
    final caught = await _storage.getCaughtMap('pokopia', ids);
  ...
  Future<void> _toggleCaught(int speciesId) async {
    ...
    await _storage.setCaught('pokopia', speciesId, newVal);
  }
...
class _HabitatImage extends StatelessWidget {
  final PokopiaHabitat habitat;
  final Color biomeColor;
  final String? name;
  const _HabitatImage(
      {required this.habitat, required this.biomeColor, this.name});
  ...
  Widget build(BuildContext context) {
    return Stack(
      ...
      children: [
        ...
        Container(
          decoration: BoxDecoration(
            gradient: LinearGradient(
              ...
              colors: [
                biomeColor.withOpacity(0.3),
                biomeColor.withOpacity(0.1),
              ],
            ),
          ),
        ),
        // Imagem do habitat (local) com fallback
        Positioned.fill(
          child: Image.asset(
            habitat.imageAsset,
            fit: BoxFit.cover,
              ...
              child: Column(
                ...
                children: [
                  Icon(Icons.image_outlined,
                      size: 48,
                      color: biomeColor.withOpacity(0.5)),
                  const SizedBox(height: 8),
                  Text(
                    'Imagem não disponível',
                    style: TextStyle(
                        fontSize: 11,
                        color: biomeColor.withOpacity(0.7)),
                  ),
                ],
              ),
            ),
        ...
        // Gradiente escuro embaixo para o título legível
        Positioned(
          bottom: 0,
          left: 0,
          right: 0,
...
class _BiomeBadge extends StatelessWidget {
  final List<String> biomes;
  final Color color;
  const _BiomeBadge({required this.biomes, required this.color});
  ...
  Widget build(BuildContext context) {
    return Wrap(
      spacing: 6,
      runSpacing: 6,
      children: biomes.map((biome) => Container(
