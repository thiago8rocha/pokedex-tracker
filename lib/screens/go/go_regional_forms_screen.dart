...
// Tipos verificados individualmente por forma. Fonte: Bulbapedia (mar/2026).
// spriteKey null = usa sprite base do ID (ex: Unown, Vivillon, Castform).
...
class _GoRegionalFormsScreenState extends State<GoRegionalFormsScreen> {
  ...
  Future<void> _openDetail(BuildContext ctx, _FormEntry form) async {
    ...
    final folder = st == 'pixel' ? 'pixel' : 'artwork';
    final key    = form.spriteKey ?? '${form.id}';
    const base   = 'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon';

    final pokemon = Pokemon(
      id: form.id, entryNumber: form.id, name: form.name,
      types: form.types,
...
class _RegionalTab extends StatelessWidget {
  ...
  static const _sections = [
    ...
    _FormSection('Paldea', [
      ...
      _FormEntry(195, 'Quagsire',                 ['poison','ground'],   spriteKey: '195_PALDEA'),
    ]),
  ];
...
class _VariantsTab extends StatelessWidget {
  ...
  static const _sections = [
    ...
    _FormSection('Vivillon (20 padrões)', [
      _FormEntry(666, 'Vivillon (vários padrões)', ['bug','flying']),
    ]),
...
class _OtherFormsTab extends StatelessWidget {
  ...
  static const _sections = [
    _FormSection('Castform (4 formas)', [
      ...
      _FormEntry(351, 'Castform Sunny',  ['fire']),
      _FormEntry(351, 'Castform Rainy',  ['water']),
      _FormEntry(351, 'Castform Snowy',  ['ice']),
    ]),
